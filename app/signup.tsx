...
export default function SignUpScreen() {
    ...
    // --- Fetch Parishes when country changes ---
    useEffect(() => {
        ...
        console.log('Fetching parishes for countryId:', countryValue);

        fetch(`${BASE_URL}/parishes/?countryId=${countryValue}`)
            .then(res => res.json())
            .then(data => {
                console.log('Parishes response:', data); // Debug log
                if (!Array.isArray(data)) {
                    console.error('Parishes data is not an array:', data);
                    setParishes([]);
                    return;
                }
    ...
    // --- Fetch Cities when parish changes ---
    useEffect(() => {
        if (parishValue === null) {
            setCities([]);
            setCityValue(null);
    ...
    const handleSubmit = () => {
        ...
        fetch(`${BASE_URL}/accounts/add`, {  // <- removed trailing slash
            method: 'POST',
            ...
            renderItem={() => (
                <View style={styles.innerContainer}>
                    ...
                    <DropDownPicker
                        ...
                        zIndexInverse={5000}
                        style={styles.dropdown}
                        dropDownContainerStyle={{ backgroundColor: '#fafafa', maxHeight: 250 }} // <- scrollable height
                        scrollViewProps={{ nestedScrollEnabled: true }} // <- enable scroll inside ScrollView
                        containerStyle={{ marginBottom: 10 }}
                    />
                    ...
                    <DropDownPicker
                        ...
                        zIndexInverse={1000}
                        style={styles.dropdown}
                        dropDownContainerStyle={{ backgroundColor: '#fafafa', maxHeight: 200 }} // scrollable
